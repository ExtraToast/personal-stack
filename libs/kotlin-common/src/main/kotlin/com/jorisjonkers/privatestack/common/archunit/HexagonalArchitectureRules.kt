...
object HexagonalArchitectureRules {
    ...
    val NO_CIRCULAR_DEPENDENCIES: ArchRule = slices()
        .matching("..(*)..")
    ...
    val LAYERED_ARCHITECTURE: ArchRule = layeredArchitecture()
        ...
        .layer("Messaging").definedBy("..infrastructure.messaging..")
        .layer("Security").definedBy("..infrastructure.security..")
        .layer("Integration").definedBy("..infrastructure.integration..")
        .layer("Config").definedBy("..config..")
        .whereLayer("Web").mayOnlyAccessLayers("Application", "Domain", "Config")
        .whereLayer("Application").mayOnlyAccessLayers("Domain")
        .whereLayer("Persistence").mayOnlyAccessLayers("Domain", "Config")
        .whereLayer("Messaging").mayOnlyAccessLayers("Domain", "Application", "Config")
        .whereLayer("Security").mayOnlyAccessLayers("Domain", "Application", "Config")
        .whereLayer("Integration").mayOnlyAccessLayers("Domain", "Config")
        .whereLayer("Domain").mayNotAccessAnyLayer()
        .because("strict layered architecture must be respected (ADR-006, ADR-013 rule 7)")

    val DOMAIN_MUST_NOT_DEPEND_ON_SPRING: ArchRule = noClasses()
    ...
    val CONTROLLERS_MUST_NOT_ACCESS_REPOSITORIES: ArchRule = noClasses()
    ...
    val COMMANDS_MUST_NOT_DEPEND_ON_WEB_OR_INFRA: ArchRule = noClasses()
    ...
    val DOMAIN_MUST_NOT_DEPEND_ON_INFRASTRUCTURE: ArchRule = noClasses()
        .that().resideInAPackage("..domain..")
        .should().dependOnClassesThat().resideInAPackage("..infrastructure..")
        .because("domain objects must not depend on infrastructure (ADR-013 rule 6)")

    val ALL_RULES: List<ArchRule> = listOf(
