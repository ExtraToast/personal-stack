package com.jorisjonkers.personalstack.assistant.application.setup
...
class SetupGuideServiceTest {
    ...
    fun `renders all placeholders for an ssh remote`() {
        val md = service.render(link(repoUrl = "git@github.com:ExtraToast/personal-stack.git"))
        assertThat(md).contains("Deploy-key setup for `auth-service`")
        assertThat(md).contains("git@github.com:ExtraToast/personal-stack.git")
        assertThat(md).contains("secret/data/agents/projects/")
        assertThat(md).contains("https://github.com/ExtraToast/personal-stack/settings/keys")
