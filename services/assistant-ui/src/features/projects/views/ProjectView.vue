...
async function onAttach(link: GithubLink, body: { privateKeyOpenssh: string; publicKeyOpenssh: string; knownHosts: string }): Promise<void> {
...
<template>
  <div class="max-w-5xl mx-auto p-6">
    ...
    <header class="mb-6">
      ...
      <p v-if="store.activeProject" class="text-xs text-gray-500 font-mono mt-1">project:{{ store.activeProject.slug }}</p>
      <p v-if="store.activeProject?.description" class="text-sm text-gray-400 mt-2">{{ store.activeProject.description }}</p>
    </header>
    ...
    <section class="mb-6">
      <div class="flex items-center justify-between mb-3">
        ...
        >{{ showAdd ? 'Cancel' : 'Add repo' }}</button>
      </div>
        ...
        <li v-for="link in store.links" :key="link.id" class="rounded-lg border border-gray-700 bg-surface-darker p-4">
          <div class="flex items-baseline justify-between mb-1">
            ...
            <div class="flex gap-2">
              ...
              >Rotate key</button>
              <button
                class="rounded text-xs text-red-400 hover:bg-red-500/10 px-3 py-1"
          ...
          <div v-if="link.deployKeyFingerprint" class="text-xs text-gray-400 font-mono mt-1">
            ...
            <span class="text-gray-500 ml-2">
              attached {{ link.deployKeyAddedAt }}
            </span>
          </div>
          <div v-else class="text-xs text-yellow-400 mt-1">No deploy key attached yet — click "Attach key" to follow the setup guide.</div>
