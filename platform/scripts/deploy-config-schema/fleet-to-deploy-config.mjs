...
function placementIntent(fleet) {
  ...
  return {
    ...
    gpu_preferences: sortObject(
      Object.fromEntries(
        Object.entries(intent.gpu_specific ?? {}).filter(([service]) => services.has(service)),
      ),
    ),
  };
}
...
function routeRules(fleet) {
  ...
  if (edgeServices.has("assistant-api")) {
    routes.push(route(edgeServices.get("assistant-api"), {
      path_prefixes: ["/api/"],
  ...
  if (edgeServices.has("knowledge-api")) {
    routes.push(route(edgeServices.get("knowledge-api"), {
