...
test('build-pi-image targets the host specific sd image output', async () => {
  ...
  const inventoryStub = await writeExecutable(
    path.join(dir, 'inventory-pi-image'),
    `
#!/usr/bin/env bash
cat <<'EOF'
...
`,
  ...
  const nixStub = await writeExecutable(
    path.join(dir, 'nix-pi-image-stub'),
    `
#!/usr/bin/env bash
printf '%s\\n' "$@" > "${nixLog}"
`,
...
test('build-pi-image rejects non arm hosts', async () => {
  const dir = await tempDir()
  const inventoryStub = await writeExecutable(
    path.join(dir, 'inventory-non-pi-image'),
    `
#!/usr/bin/env bash
cat <<'EOF'
...
`,
  ...
  const nixStub = await writeExecutable(
    path.join(dir, 'nix-non-pi-image-stub'),
    `
#!/usr/bin/env bash
