...
test("manifest pins every checked-in skill hook setting and installer file", async () => {
  ...
  assertSameMembers(manifestSkillPaths, actualRepoSkillPaths, "every checked-in Claude/Codex skill must be listed in the agent-kit manifest");
  ...
  assertSameMembers(manifestHookPaths, actualRepoHookPaths, "every checked-in Claude/Codex hook must be listed in the agent-kit manifest");
  ...
  assertSameMembers(manifestCommandPaths, actualRepoCommandPaths, "every checked-in Claude command must be listed in the agent-kit manifest");

  const pinnedPaths = collectPinnedPaths(manifest);
  for (const required of [".claude/settings.json", ".codex/hooks.json", "services/knowledge-api/src/main/resources/installer/install.sh"]) {
...
test("Spec Kit Claude commands and Codex skills stay in one to one parity", async () => {
  const commandNames = new Set([...await repoCommandPaths()].map((item) => item.substring(item.lastIndexOf("/speckit.") + 9).replace(/\.md$/, "")));
  const skillNames = new Set([...await repoSkillPaths()].map((item) => /^\.agents\/skills\/speckit-([^/]+)\/SKILL\.md$/.exec(item)?.[1]).filter(Boolean));
  assertSameMembers(skillNames, commandNames, "each /speckit.<command> must have a matching Codex speckit-<command> skill");
});
...
test("installer managed surfaces are listed in the manifest", async () => {
  ...
  const installedSkillNames = new Set([...installer.matchAll(/\$\{(?:CODEX_)?SKILLS_DIR}\/([^/]+)\/SKILL\.md/g)].map((match) => match[1]));
  const manifestInstallerSkillNames = new Set(manifestItems("skills").filter((item) => item.installer).map((item) => item.name));
  assertSameMembers(manifestInstallerSkillNames, installedSkillNames, "every installer-managed skill must be visible in the agent-kit manifest");

  const installedHookNames = new Set([...installer.matchAll(/\$\{(?:CODEX_)?HOOKS_DIR}\/([^"]+\.sh)/g)].map((match) => match[1]));
  const manifestInstallerHookNames = new Set(
    manifestItems("hooks").flatMap((hook) => [hook.installer?.target_path, hook.installer?.codex_target_path].filter(Boolean).map((item) => item.substring(item.lastIndexOf("/") + 1))),
  );
  assertSameMembers(manifestInstallerHookNames, installedHookNames, "every installer-managed hook must be visible in the agent-kit manifest");
});
...
test("installer dry-run covers Claude and Codex managed surfaces", async () => {
  ...
  const result = await runProcess("bash", [installer, "--agent", "all", "--dry-run"], { env: { CLAUDE_CONFIG_DIR: claudeHome, CODEX_HOME: codexHome } });
...
test("installer agent selection covers Spec Kit commands and Codex skills", async () => {
  ...
  for (const agent of ["claude", "codex", "all"]) {
    ...
    assert.equal(dryRunResult.stdout.includes(`would write ${claudeHome}/commands/speckit.analyze.md`), agent !== "codex");
    assert.equal(dryRunResult.stdout.includes(`would write ${codexHome}/skills/speckit-analyze/SKILL.md`), agent !== "claude");
...
test("installer writes parseable Codex hooks and uninstalls managed files", async () => {
  ...
  assertContains(await readFile(path.join(claudeHome, ".knowledge-system-version"), "utf8"), "scope=user", "managed:", "hooks/user-prompt-submit-recall.sh", "commands/speckit.analyze.md");
  assertContains(await readFile(path.join(codexHome, ".knowledge-system-version"), "utf8"), "agent=codex", "scope=user", "hooks.json", "skills/speckit-analyze/SKILL.md");
  ...
  assert.deepEqual(hookCommands(codexHooks, "UserPromptSubmit"), [path.join(codexHome, "hooks/kb-user-prompt-recall.sh")]);
...
test("installer project scope writes claude and codex files under project root", async () => {
  ...
  const env = { AGENT_KIT_PROJECT_ROOT: projectRoot, CLAUDE_CONFIG_DIR: ignoredClaudeHome, CODEX_HOME: ignoredCodexHome };
  ...
  for (const file of [...managedFiles, ...specifySeedFiles]) assert.ok(existsSync(file), `project-scope installer wrote ${file}`);
  assert.ok((await import("node:fs")).statSync(path.join(projectRoot, ".specify/scripts/bash/check-prerequisites.sh")).mode & 0o111);
  assert.equal((await readFile(path.join(projectRoot, ".specify/memory/constitution.md"), "utf8")).trimEnd(), (await readRepoText(".specify/memory/constitution.md")).trimEnd());
  ...
  const uninstallResult = await runProcess("bash", [installer, "--agent", "all", "--scope", "project", "--uninstall"], { env });
  assert.equal(uninstallResult.exitCode, 0, uninstallResult.stderr);
  for (const file of managedFiles) assert.equal(existsSync(file), false, `project-scope uninstall removed ${file}`);
  for (const file of specifySeedFiles) assert.ok(existsSync(file), `project-scope uninstall should preserve project-owned Spec Kit seed ${file}`);
});
...
test("installed recall hooks parse payloads dedupe and honor allowlist", async () => {
  ...
  const hookEnvironment = { ...installEnvironment, KB_BEARER_TOKEN: "test-token", KB_URL: "http://knowledge.local", CURL_CAPTURE_FILE: curlLog, PATH: `${fakeBin}:${process.env.PATH}` };
  ...
  assertContains(await readFile(path.join(codexHome, "hooks/pre-tool-use-edit-recall.sh"), "utf8"), "Related captures for this file");
...
test("installed capture hooks parse commits and stop digest sessions", async () => {
  ...
  const hookEnvironment = { ...installEnvironment, KB_BEARER_TOKEN: "test-token", KB_URL: "http://knowledge.local", PATH: `${fakeBin}:${process.env.PATH}` };
  ...
  const codexCommit = await runProcess(path.join(codexHome, "hooks/pre-tool-use-git-commit-capture.sh"), [], {
    env: { ...commitEnvironment, KB_AUTO_MCP_CLIENT_NAME: "Codex", KB_AUTO_MCP_SOURCE: "codex:auto-capture:git-commit" },
  ...
  await writeFile(claudeTranscript, `{"role":"user","content":"Claude stop transcript should create a durable lesson."}\n`);
  ...
  assert.deepEqual(claudeStopPayloads.map(toolName), ["knowledge.digest_transcript", "knowledge.recall", "knowledge.capture_lesson"]);
  ...
  assert.equal((await readFile(path.join(claudeHome, "state/sessions/claude-stop-session/digest-budget"), "utf8")).trim(), "0");
  ...
  await writeFile(codexTranscript, `{"source":"user","message":"Codex stop transcript should fall back to project scope."}\n`);
  ...
  const codexStop = await runProcess(path.join(codexHome, "hooks/kb-stop-digest.sh"), [], {
    env: { ...hookEnvironment, CODEX_HOME: codexHome, CURL_CAPTURE_FILE: codexStopCurlLog, KB_DIGEST_MAX_CAPTURES: "1" },
  ...
  assert.deepEqual(codexStopPayloads.map(toolName), ["knowledge.digest_transcript", "knowledge.recall", "knowledge.capture_lesson"]);
  ...
  assert.equal((await readFile(path.join(codexHome, "state/sessions/codex-stop-session/digest-budget"), "utf8")).trim(), "0");
});
...
test("installed hooks stay silent when disabled or unauthenticated", async () => {
  ...
  await writeFile(claudeTranscript, `{"role":"user","content":"Claude stop transcript should not call MCP when disabled."}\n`);
  await writeFile(codexTranscript, `{"source":"user","message":"Codex stop transcript should not call MCP when disabled."}\n`);
  const invocations = [
    [`{"user_prompt":"Recall should stay silent when disabled."}`, path.join(claudeHome, "hooks/user-prompt-submit-recall.sh"), {}],
    [`{"user_prompt":"Recall should stay silent when disabled."}`, path.join(codexHome, "hooks/kb-user-prompt-recall.sh"), {}],
    [`{"tool_input":{"file_path":"platform/agents/kit/manifest.yaml"}}`, path.join(claudeHome, "hooks/pre-tool-use-edit-recall.sh"), {}],
    [`{"tool_input":{"file_path":"platform/agents/kit/manifest.yaml"}}`, path.join(codexHome, "hooks/pre-tool-use-edit-recall.sh"), { KB_AUTO_MCP_HOME: codexHome }],
    [`{"tool_name":"Bash","tool_input":{"command":"git commit -m \\"Capture hook silence\\""}}`, path.join(claudeHome, "hooks/pre-tool-use-git-commit-capture.sh"), {}],
    [`{"tool":{"name":"Bash","input":{"cmd":"git commit -m 'Capture Codex silence'"}}}`, path.join(codexHome, "hooks/pre-tool-use-git-commit-capture.sh"), { KB_AUTO_MCP_CLIENT_NAME: "Codex", KB_AUTO_MCP_SOURCE: "codex:auto-capture:git-commit" }],
    [JSON.stringify({ session_id: "silent-claude", transcript_path: claudeTranscript }), path.join(claudeHome, "hooks/stop-session-digest.sh"), {}],
    [JSON.stringify({ thread_id: "silent-codex", transcriptPath: codexTranscript }), path.join(codexHome, "hooks/kb-stop-digest.sh"), { CODEX_HOME: codexHome }],
  ];
  const hookEnvironment = { ...installEnvironment, KB_URL: "http://knowledge.local", CURL_CAPTURE_FILE: curlLog, PATH: `${fakeBin}:${process.env.PATH}` };
  for (const [input, command, env] of invocations) {
    const result = await runProcess(command, [], { env: { ...hookEnvironment, ...env, KB_BEARER_TOKEN: "test-token", KB_AUTO_MCP_DISABLED: "1" }, input });
    assert.equal(result.exitCode, 0, result.stderr);
  }
...
test("hook settings reference only manifest hooks", async () => {
  ...
  for (const file of [".claude/settings.json", ".codex/hooks.json"]) {
    ...
    for (const match of text.matchAll(/\.(?:claude|codex)\/hooks\/[A-Za-z0-9._-]+\.sh/g)) settingsHookPaths.add(match[0]);
  }
  assertSameMembers(settingsHookPaths, manifestHookPaths, "checked-in hook settings must not reference scripts outside the agent-kit manifest");
});
...
test("hook tool calls use canonical knowledge mcp names", async () => {
  ...
  for (const file of hookFiles) {
    const text = await readRepoText(file);
    for (const match of text.matchAll(/["']name["']\s*:\s*["'](knowledge\.[A-Za-z_]+)["']/g)) referencedTools.add(match[1]);
    for (const match of text.matchAll(/knowledge_(?:recall|capture_lesson|capture_decision|digest_transcript)/g)) legacyNames.push(`${file}:${match[0]}`);
  }
  for (const tool of referencedTools) assert.ok(knownKnowledgeTools.has(tool), `${tool} should be a canonical knowledge tool`);
  assert.deepEqual(legacyNames, []);
  for (const hook of manifestItems("hooks")) {
    for (const tool of hook.mcp_tools ?? []) assert.ok(knownKnowledgeTools.has(tool), `${hook.name} declares canonical tool ${tool}`);
  }
});
...
test("recall injection hooks fall back to fast mode", async () => {
  ...
  for (const file of [".claude/hooks/pre-tool-use-edit-recall.sh", ".codex/hooks/pre-tool-use-edit-recall.sh"]) {
    const script = await readRepoText(file);
    assertContains(script, `[ "\${mode}" != "fast" ]`, `call_recall "\${query}" "\${limit}" fast "\${scope}"`, `args["scope"] = sys.argv[4]`);
  }
...
test("renderer templates are declared in the manifest", async () => {
  ...
  assert.equal(new Set(managedPathList).size, managedPathList.length, "renderer managed paths must not contain duplicates");
  assertSameMembers(templatePaths, managedPaths, "renderer templates must match manifest managed paths exactly");
  for (const managedPath of managedPaths) assert.ok(pinnedPaths.has(managedPath), `${managedPath} is pinned by manifest sha256`);
});
...
test("renderer include partials are declared and resolvable", async () => {
  ...
  assertSameMembers(declaredPartials, referencedPartials, "renderer include partials must be explicit manifest inventory");
...
test("renderer check passes and can render templates to a temp directory", async () => {
  ...
  assert.ok((await import("node:fs")).statSync(renderer).mode & 0o111, "agent-kit renderer should be directly executable");
  ...
  if (checkResult.exitCode === 0) {
    ...
    for (const line of missingGeneratedSkillLines) assert.match(line, /^missing: \.agents\/skills\/speckit-[^/]+\/SKILL\.md$/);
    assert.equal(missingGeneratedSkillLines.length, [...await repoSkillPaths()].filter((item) => item.startsWith(".agents/skills/speckit-")).length);
  }
  ...
  for (const managedPath of rendererManagedPaths()) {
    assert.deepEqual(await readFile(path.join(outputDir, managedPath)), await readFile(rendererSourcePath(managedPath)), `rendered output for ${managedPath}`);
  }
});
...
test("portability runbook documents install scope export restore and compatibility matrix", async () => {
  ...
  assertContains(runbook, "--scope user", "--scope project", "knowledge-vault", "Postgres logical backup", "Restore Order", "Compatibility Matrix", "Claude Code", "Codex", "knowledge.recall");
});
...
test("agent kit doctor reports static health and skipped live checks", async () => {
  ...
  const result = await runProcess(renderer, ["--doctor"], {
    env: { HOME: home, CLAUDE_CONFIG_DIR: path.join(home, ".claude"), CODEX_HOME: path.join(home, ".codex"), KB_URL: "", KB_BEARER_TOKEN: "" },
  });
  assert.equal(result.exitCode, 0, result.stderr);
  assertContains(
    result.stdout,
...
test("agent kit doctor validates installed manifest versions when expected version is set", async () => {
  ...
  const result = await runProcess(renderer, ["--doctor"], {
    env: { HOME: home, CLAUDE_CONFIG_DIR: claudeHome, CODEX_HOME: codexHome, AGENT_KIT_EXPECTED_VERSION: "current", KB_URL: "", KB_BEARER_TOKEN: "" },
  });
  assert.equal(result.exitCode, 0, result.stderr);
  assertContains(
    result.stdout,
...
test("agent kit doctor can require live kb credentials", async () => {
  ...
  const result = await runProcess(renderer, ["--doctor", "--require-live-kb"], {
    env: { HOME: home, CLAUDE_CONFIG_DIR: path.join(home, ".claude"), CODEX_HOME: path.join(home, ".codex"), KB_URL: "http://knowledge.local", KB_BEARER_TOKEN: "" },
  });
  assert.equal(result.exitCode, 1, result.stdout);
  assertContains(result.stdout, "fail kb-live: KB_BEARER_TOKEN is not set; live MCP probe skipped", "summary: 3 ok, 2 warn, 1 fail");
});
...
test("agent kit doctor probes tools list and fast recall when live kb credentials exist", async (t) => {
  ...
  const server = http.createServer((request, response) => {
    ...
    request.on("end", () => {
      ...
      const out = payload.method === "tools/list"
  ...
  try {
    ...
    const result = await runProcess(renderer, ["--doctor"], {
      env: { HOME: home, CLAUDE_CONFIG_DIR: path.join(home, ".claude"), CODEX_HOME: path.join(home, ".codex"), KB_URL: `http://127.0.0.1:${port}`, KB_BEARER_TOKEN: "test-token" },
    });
    assert.equal(result.exitCode, 0, result.stderr);
    assertContains(result.stdout, `ok   kb-live: reachable at http://127.0.0.1:${port}/mcp with 2 tools; fast recall returned 1 hits`, "summary: 4 ok, 2 warn, 0 fail");
    assert.deepEqual(payloads.map((payload) => payload.method), ["tools/list", "tools/call"]);
...
test("knowledge-api installer placeholders and public routes stay exposed", async () => {
  const installer = await readRepoText("services/knowledge-api/src/main/resources/installer/install.sh");
  assertContains(installer, "readonly INSTALLER_VERSION='@VERSION@'", "readonly KB_URL='@KB_URL@'", "KB_URL=\"${KB_URL:-@KB_URL@}\"", "${KB_URL}/install.sh");
  ...
  const appRoute = fleet.ingress_intent.route_rules.find((item) => item.service === "knowledge-api" && item.name !== "knowledge-api-mcp");
...
async function installAgentKit(dir, prefix) {
  ...
  const installResult = await runProcess("bash", [repoPath(manifest.installer.path), "--agent", "all"], { env: installEnvironment });
  assert.equal(installResult.exitCode, 0, installResult.stderr);
...
async function repoSkillPaths() {
  ...
  for (const base of [".agents/skills", ".claude/skills"]) {
    ...
    for (const file of await filesUnder(root)) if (path.basename(file) === "SKILL.md") paths.push(`${base}/${relativePath(root, file)}`);
  }
...
async function repoCommandPaths() {
  ...
  return new Set((await filesUnder(root)).filter((file) => path.basename(file).startsWith("speckit.") && file.endsWith(".md")).map((file) => `.claude/commands/${relativePath(root, file)}`));
}
...
function assertAgentGapIsExplicit(label, node) {
  ...
  for (const agent of missingAgents) assert.ok(node.unsupported?.[agent]?.trim(), `${label} unsupported reason for ${agent}`);
}

async function canonicalKnowledgeToolNames() {
  const text = await readRepoText("services/knowledge-api/src/test/kotlin/com/jorisjonkers/personalstack/knowledge/mcp/McpToolsTest.kt");
