...
test('every spring vault database role referenced by a service has a matching bootstrap role', async () => {
  ...
  assert.ok(
    roles.length > 0,
    'expected to find at least one spring.cloud.vault.database.role declaration in a service application.yml',
  )
...
async function collectDeclaredVaultDatabaseRoles() {
  const serviceFiles = (await filesUnder(repoPath('services'))).filter(
    (file) =>
      path.basename(file) === 'application.yml' && relativePath(repoRoot, file).includes('/src/main/resources/'),
  )
  const roles = []
