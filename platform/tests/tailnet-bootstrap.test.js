...
test("platform inventory no longer models headscale as a planned service", async () => {
  ...
  const exposedServices = [...fleet.exposure_intent.public, ...fleet.exposure_intent.public_and_lan, ...fleet.exposure_intent.internal_only, ...fleet.exposure_intent.lan_only];
...
test("bootstrap docs describe Tailscale admin console auth key flow", async () => {
  ...
  assertContains(tailnetPlaybook, "one-off auth key", "Tailscale admin console", "bootstrap-tailnet.sh <node-name>", "MagicDNS");
});
...
test("tailnet bootstrap helper expects an auth key and runs tailscale up remotely", async () => {
  ...
  assertContains(helperScript, "TS_AUTH_KEY", "PLATFORM_SSH_IDENTITY_FILE", "BOOTSTRAP_SSH_HOST", "require_platform_ssh_identity_file_if_set", "tailscale up", "--auth-key=", "--hostname=", "tailscale status");
});
