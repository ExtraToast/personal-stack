...
export function validateFleet(fleet) {
  ...
  assert.ok(k8s.control_plane_token_file.startsWith("/"), "cluster kubernetes control_plane_token_file must be an absolute path");
  assert.ok(k8s.worker_join_token_file.startsWith("/"), "cluster kubernetes worker_join_token_file must be an absolute path");
  ...
  for (const serviceName of fleet.access_intent?.sso_protected ?? []) {
    ...
    assert.ok(externallyExposedServices.has(serviceName), `sso protected service ${serviceName} must be externally exposed`);
  }
  ...
  for (const serviceName of externallyExposedServices) {
    assert.ok(serviceName in (fleet.access_intent?.host_labels ?? {}), `externally exposed service ${serviceName} must declare a host label`);
  }

  for (const [serviceName, backend] of Object.entries(fleet.ingress_intent?.kubernetes_backends ?? {})) {
    assert.ok(knownKubernetesServices.has(serviceName), `ingress backend for service ${serviceName} references unknown kubernetes service intent`);
    assert.ok(externallyExposedKubernetesServices.has(serviceName), `ingress backend for service ${serviceName} must target an externally exposed kubernetes service`);
    validateBackend("ingress", serviceName, backend, true, fleet);
  }
  for (const serviceName of externallyExposedKubernetesServices) {
    assert.ok(serviceName in (fleet.ingress_intent?.kubernetes_backends ?? {}), `externally exposed kubernetes service ${serviceName} must declare an ingress backend`);
  }

  for (const [serviceName, origin] of Object.entries(fleet.ingress_intent?.wan_origin_overrides ?? {})) {
    assert.ok(externallyExposedKubernetesServices.has(serviceName), `wan origin override for service ${serviceName} must target an externally exposed kubernetes service`);
    assert.ok(
  ...
  for (const [serviceName, backend] of Object.entries(fleet.monitoring_intent?.kubernetes_backends ?? {})) {
    assert.ok(knownKubernetesServices.has(serviceName), `monitoring backend for service ${serviceName} references unknown kubernetes service intent`);
    assert.ok(!externallyExposedKubernetesServices.has(serviceName), `monitoring backend for service ${serviceName} duplicates an ingress backend; probe it via ingress_intent instead`);
...
function validateBackend(kind, serviceName, backend, ingress, fleet) {
  assert.ok(String(backend.namespace ?? "").trim().length > 0, `${kind} backend namespace for service ${serviceName} must not be blank`);
  assert.ok(String(backend.service ?? "").trim().length > 0, `${kind} backend service name for service ${serviceName} must not be blank`);
  ...
  if (backend.health) {
    ...
    assert.ok(["http", "tcp"].includes(health.type), `${kind === "monitoring" ? "monitoring " : ""}health type for service ${serviceName} must be http or tcp`);
    if (health.type === "tcp") {
      assert.equal(health.path, "/", `tcp ${kind === "monitoring" ? "monitoring " : ""}health for service ${serviceName} must not set a path`);
      assert.equal(health.expected_status, undefined, `tcp ${kind === "monitoring" ? "monitoring " : ""}health for service ${serviceName} must not set expected_status`);
    }
    assert.ok(health.path.startsWith("/"), `${kind === "monitoring" ? "monitoring " : ""}health path for service ${serviceName} must start with /`);
    if (health.port !== undefined) assert.ok(health.port > 0, `${kind === "monitoring" ? "monitoring " : ""}health port for service ${serviceName} must be positive`);
    if (ingress && health.probe_strategy !== undefined) {
      assert.ok(["internal", "external", "both"].includes(health.probe_strategy), `health probe_strategy for service ${serviceName} must be internal, external, or both`);
      if (["external", "both"].includes(health.probe_strategy)) {
        assert.ok(serviceName in (fleet.access_intent?.host_labels ?? {}), `health probe_strategy ${health.probe_strategy} for service ${serviceName} requires a host label`);
      }
    }
    if (!ingress) {
      assert.ok(health.probe_strategy === undefined || health.probe_strategy === "internal", `monitoring health probe_strategy for service ${serviceName} must be internal (monitoring targets have no external host)`);
    }
  }
  for (const probe of backend.extra_probes ?? []) {
    ...
    assert.ok(["http", "tcp"].includes(type), `extra probe ${probe.name} for service ${serviceName} must be http or tcp`);
    if (type === "tcp") {
      assert.equal(probePath, "/", `tcp extra probe ${probe.name} for service ${serviceName} must not set a path`);
      assert.equal(probe.expected_status, undefined, `tcp extra probe ${probe.name} for service ${serviceName} must not set expected_status`);
    }
...
export function yamlDocs(text) {
  return YAML.parseAllDocuments(text).map((doc) => doc.toJSON()).filter(Boolean);
}
