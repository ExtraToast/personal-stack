...
class PlatformAgentMcpFluxTest {
    ...
    fun `agent runner wires github app token into gh github mcp and git push`() {
        ...
        assertThat(dockerfile)
            ...
            .contains("COPY platform/agents/kit/templates/repo/.specify /opt/agent-kit/sdd")
            .contains("AGENT_KIT_SDD_SOURCE=/opt/agent-kit/sdd AGENT_RUNNER_ENTRYPOINT_SELF_TEST=speckit-seed")
            .contains(".agent-kit-sdd-seed.sha256")

        assertThat(entrypoint)
            .contains("credential.helper agent-gh-app")
            .contains("credential.useHttpPath true")
            .contains("AGENT_KIT_SDD_SOURCE=\"\${AGENT_KIT_SDD_SOURCE:-/opt/agent-kit/sdd}\"")
            .contains("AGENT_KIT_SDD_MARKER_FILE=\"\${AGENT_KIT_SDD_MARKER_FILE:-.agent-kit-sdd-seed.sha256}\"")
            .contains("speckit_seed()")
            .contains("AGENT_RUNNER_ENTRYPOINT_SELF_TEST:-}\" = \"speckit-seed\"")
            .contains("for _speckit_git_dir in \"\${WORKSPACE_ROOT}\"/*/.git")
            .contains("speckit_seed \"\${_speckit_git_dir%/.git}\"")
            .contains("speckit_seed_workspace\n\n# The gateway shares this Pod's memory cgroup")
